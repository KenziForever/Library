...
class OldListView(View):
    def get(self, request):
        Old = models.Product.objects.select_related().filter(tags__name="Для старых").order_by("-id")
        context = {"Old": Old}
...
class NewListView(View):
    def get(self, request):
        News = models.Product.objects.select_related().filter(tags__name="Для молодых").order_by("-id")
        context = {"News": News}
...
class BabyListView(View):
    def get(self, request):
        Baby = models.Product.objects.select_related().filter(tags__name="Для детей").order_by("-id")
        context = {"Baby": Baby}
